...
def _generate_answer(question: str, context: list[dict], cfg: RAGConfig, llm: BaseChatModel) -> str:
...
def _eval_variant(
    ...
    for item in gold:
        ...
        t0 = perf_counter()
        try:
            context = retrieve_context(question, doc_ids, cfg, vs, answer_llm)
        except Exception:
        ...
        if judge_on:
            try:
                answer = _generate_answer(question, context, cfg, answer_llm)
                row["answer"] = answer
                verdict = judge_mod.judge_answer(question, [c["text"] for c in context], answer, judge_llm)
                if verdict:
